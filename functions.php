...
function lbs_get_catalog_settings() {
	...
	// Get fix capitalization
	if ( isset( $options['lbs_cat_FixCapitalization'] ) ) {
		$cat['fixcaps'] = $options['lbs_cat_FixCapitalization'];
	} else {
		$cat['fixcaps'] = TRUE;
	}
				...
				switch ( $cat['sys'] ) {
					...
					case 'pika':
						if ( ( $wsapi === 'pika-api' || $wsapi === 'aspen-api' ) && 'false' === $list_input ) {
							// URL format for Aspen/Pika API results
							$cat_url = $cat['domain'] . "/GroupedWork/" . $itemID;
						} elseif ( strlen( $itemID ) >= 10 ) {
							// URL format for ISBN/UPC
							$cat_url = $cat['domain'] . "/Search/Results?lookfor0[]=" . $itemID . "&type0[]=ISN";
						}  else {
							// URL format for item record numbers
							$cat_url = $cat['domain'] . "/Search/Results?lookfor=" . $itemID . "&type=id";
						}
						break;
					case 'polaris':
						if ( $cat['profile'] ) {
					...
					case 'vega':
						if ( strlen( $itemID ) === 12 ) {
							$cat_url = $cat['domain'] . "/search/card?upc=" . $itemID;
						} else {
							$cat_url = $cat['domain'] . "/search/card?isbn=" . $itemID;
						}
						break;
					case 'webpac':
						if ( $cat['profile'] ) {
	...
	switch ( $cat['cdn'] ) {
		...
		case 'pika':
			if ( ( $wsapi === 'pika-api' || $wsapi === 'aspen-api' ) && 'false' === $list_input ) {
				...
				// URL format for ISBN/UPC
				$cat_img_url = $cat['domain'] . "/bookcover.php?size=medium&isn=" . $itemID . "&upc=" . $itemID;
			} else {
				// URL format for item record numbers
				$cat_img_url = $cat['domain'] . "/bookcover.php?size=medium&id=" . $itemID;
			}
			break;
		case 'syndetics':
			if ( strlen( $itemID ) === 12 ) {
	...
	switch ( $api_meta['wsapi'] ) {
		case 'bibliocommons-rss':
			...
			foreach( $items as $item ) {
				...
				$image_url = $item->get_item_tags( '', 'image_url' )[0]['data'];

				// BiblioCommons uses Syndetics images
				// Look for ISBN in image URL
				preg_match( "/(?<=isbn=)(\d+[xX]?)/", $image_url, $item_id );

				// If no ISBN look for UPC
				if ( empty( $item_id ) ) {
					preg_match( "/(?<=upc=)(\d+)/", $image_url, $item_id );
				}

				// If there's an item ID get more item data
				if ( $item_id ) {
					$title = lbs_trim_item_title( $item->get_title() );

					// Get item creator
					$creator = $item->get_item_tags( 'http://purl.org/dc/elements/1.1/', 'creator' )[0]['data'];

					// If no creator data, just use title
					if ( empty( $creator ) ) {
						$alt = $title;
					} else {
...
function lbs_settings_obj() {
	$settings['catalog'] = array(
		...
		'fields'      => array(
			...
			array(
				'id'          => 'cat_FixCapitalization',
				'label'       => __( 'Fix Capitalization', 'library-bookshelves' ),
				'description' => __( 'Correct the capitalization of item title and author metadata retrieved from web services.', 'library-bookshelves' ),
				'type'        => 'checkbox',
				'default'     => 'on',
			),
			array(
