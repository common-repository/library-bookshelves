 ...
 public function add_items_metabox() {
		...
		<div id="api-input" style="display: <?php echo ( 'false' === $list_input ? "block" : "none" ); ?>">
			...
			<select id="wsapi" name="wsapi" onchange="service(this.value)">
				...
				<option value="bibliocommons-rss" <?php selected( $wsapi, 'bibliocommons-rss' ); ?> >BiblioCommons RSS Feed</option>
		...
		function service( value ) {
			switch ( value ) {
				...
				case 'koha-rss':
				case 'sirsi-rss':
					<?php $notice = esc_html__("Enter an RSS feed URL from your OPAC. If no items appear in the Bookshelf it's likely your server did not respond within the 10-second timeout threshold. If this occurs try again later, try a simpler query, or one which yields fewer results.", 'library-bookshelves' ); ?>
					jQuery( '#serviceNotice' ).html("<?php echo $notice; ?>");
					jQuery( '#ws-key' ).hide();
					jQuery( '#ws-secret' ).hide();
					jQuery( '#ws-token' ).hide();
